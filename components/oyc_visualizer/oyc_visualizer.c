/*
 * oyc_visualizer —— AI 音乐氛围灯引擎（xiaozhi-wifi-audio-light 专用）
 *
 * 数据流（单任务，低优先级）：
...
void oyc_visualizer_set_mode(led_mode_t mode) {
    s_auto_follow = false;
...
void oyc_visualizer_set_mode_auto(led_mode_t mode) {
...
void oyc_visualizer_set_auto_follow(bool follow) {
    s_auto_follow = follow;
    ESP_LOGI(TAG, "自动跟随设备状态: %s", follow ? "开" : "关");
}

bool oyc_visualizer_is_auto_follow(void) {
    return s_auto_follow;
}

void oyc_visualizer_set_brightness(int percent) {
...
void oyc_visualizer_set_post(float gamma, uint8_t noise_gate, float afterimage) {
...
void oyc_visualizer_set_fx(const led_fx_t *fx) {
...
void oyc_visualizer_get_spectrum(uint8_t *out32) {
...
led_mode_t oyc_visualizer_get_mode(void) {
    return s_current_mode;
}

int oyc_visualizer_get_brightness(void) {
    return led_get_brightness();
}

int oyc_visualizer_get_bpm(void) {
    return led_get_beat()->bpm;
}

void oyc_visualizer_set_audio_source(oyc_audio_src_t src) {
    if (src == OYC_AUDIO_SRC_WIFI && !s_wifi_rx_up) {
    ...
    ESP_LOGI(TAG, "音频源切换: %s (端口 %d)",
             src == OYC_AUDIO_SRC_WIFI ? "WiFi 推流" : "麦克风", WIFI_AUDIO_UDP_PORT);
}

oyc_audio_src_t oyc_visualizer_get_audio_source(void) {
    return s_audio_src;
}

bool oyc_visualizer_audio_streaming(void) {
    return s_audio_src == OYC_AUDIO_SRC_WIFI && wifi_audio_streaming();
}
...
void oyc_visualizer_set_ai_audio_cb(oyc_ai_audio_cb_t cb) {
    s_ai_cb = cb;
}
...
void oyc_visualizer_feed(const int16_t *pcm, int samples) {
    if (!s_started) return;
...
esp_err_t oyc_visualizer_start(int gpio, int led_num, int brightness_percent) {
